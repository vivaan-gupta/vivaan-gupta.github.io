...
2. <a id="continuity"></a> **Continuity** : $$\forall a,b,c\in S : \{\alpha ~⎮~ \alpha a + (1-\alpha)b \succcurlyeq c\}$$ and $$\{\alpha ~⎮~ c \succcurlyeq \alpha a + (1-\alpha)b \}$$, i.e $$\succcurlyeq$$-upper contour and lower contour, are [closed](#closed-set)
...
4.  <a id="1.4"></a> $$\forall a,b\in S : a \succ b \implies \forall \mu\in(0,1) : a \succ \mu a + (1-\mu)b \succ b$$
  ...
  > By successively applying [1.3](#1.3), we can say that $$\forall \rho_1, \rho_2 : \rho_2>\rho_1 \implies a \succ \rho_2 a + (1-\rho_2)b \succ \rho_1 a + (1-\rho_1)b \succ b$$ 
  > [^0]    
...
5. <a id="1.5"></a> $$\forall a,a'\in S : a\sim a' \implies \mu a + (1-\mu)a' \sim a$$
  ...
  > By successively applying the same, we can say that $$\forall \rho' : a \sim \rho' a + (1-\rho') a'$$      
...
[](-----------------------------)
### Other Results
1. <a id="A.1"></a> Every interval in $$\mathbb{R}$$ is [connected](#connected-set)
 > **Proof**    
 > Let us take arbitrary interval $$Y$$   
 > Let us assume that $$Y$$ is disconnected   
 > Thus, $$\exists V, W \subseteq Y$$ such that $$V, W$$ are non-empty, disjoint, open such that $$V\cup W = Y$$    
 > Let $$a\in V$$ and $$b\in W$$, then without loss of generality let it be the case that $$a<b$$ we can say that $$V$$ is bounded above at $$b$$   
 > $$V$$ is bounded above by $$b$$, thus $$\exists z \in Y: z = \sup(V)$$   
 > $$z\in V \lor z\in W$$   
 > - $$z\in V$$   
 > Since $$V$$ is an open set all points in the set are interior points, i.e $$\forall x\in V : \exists r'>0 : B(x,r')\subseteq V$$   
 > Thus $$z\neq \sup(V)$$ as there exists a point $$z+r'\in V$$   
 > - $$z\in W$$   
 > Since $$W$$ is an open set all points in the set are interior points, i.e $$\forall x\in W : \exists r'>0 : B(x,r)\subseteq W$$    
 > Thus $$z\neq \sup(V)$$ as there exists a point $$z-r'\in W$$     
 >    
